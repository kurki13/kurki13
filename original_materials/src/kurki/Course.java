...
public class Course implements Option, Serializable {
    ...
    protected static final String COURSE_WHERE_SQL =
	"    kurssikoodi = ?\n"
	+"    AND lukukausi = ?\n"
	+"    AND lukuvuosi = ?\n"
	+"    AND tyyppi = ?\n"
    ...
    protected static final String DEFREEZE = 
    ...
    public static final String RETURN_ON_COURSE_SQL = 
    ...
    public static final String[] SET_MINMAX_SCORE_SQL =
    {"UPDATE kurssi\n"
    ...
    protected static final String[] SCORE_UPDATE_SQL =
    { "UPDATE osallistuminen \n"
    ...
    protected final String SET_PART_DEF_SQL
	="UPDATE kurssi\n"
	...
	conventions.add( new Convention( 1,
					 "Normaali",
					 "koe + laskarihyvitys+ harjoitustyˆhyvitys "
					 +"(kaikki osat vaikuttavat)." ) );
	conventions.add( new Convention( 2,
					 "Parempi ht korvaa laskarit",
					 "koe + greatest(laskarihyvitys,harjoitustyˆhyvitys) "
					 +"(laskarit voi korvata harjoitustˆill‰ - "
					 +"parempi valitaan)." ) );
	conventions.add( new Convention( 3,
					 "Tehty ht korvaa laskarit",
					 "koe + if exists harjoitustyohyvitys then "
					 +"harjoitustyˆhyvitys else laskarihyvitys "
					 +"(laskarit voi korvata harjoitustˆill‰, "
					 +"jos harjoitustˆit‰ on tehty ne ajavat laskareiden "
					 +"yli)." ) );
	conventions.add( new Convention( 4,
					 "Parempi lis‰koe korvaa laskarit",
					 "koe(1..n-1) + greatest(laskarihyvitys, "
					 +"koe(n)) laskarit voi korvata ylim‰‰r‰isell‰ "
					 +"koeteht‰v‰ll‰, joka kirjataan omaksi kokeekseen "
					 +"- parempi valitaan." ) );
	conventions.add( new Convention( 5,
					 "Tehty lis‰koe korvaa laskarit",
					 "koe(1..n-1) + if exists koe(n) then koe(n) "
					 +"else laskarihyvitys laskarit voi korvata "
					 +"ylim‰‰r‰isell‰ koeteht‰v‰ll‰, joka kirjataan "
					 +"omaksi kokeekseen - ylim‰‰r‰inen teht‰v‰ "
					 +"ajaa laskareiden yli" ) );
	conventions.add( new Convention( 6,
					 "Parempi ht korvaa muut",
					 "greatest(koe+laskarihyvitys,harjoitustyˆhyvitys) "
					 +"kokeen ja laskarit voi korvata harjoitustˆill‰." ) );
	...
	conventions.add( new Convention( 8,
					 "Ylˆsskaalattu",
					 "greatest(koe+laskarihyvitys+harjoitustyˆhyvitys, "
					 +"koe*maksimipistem‰‰r‰/koemaksimi) "
					 +"parempi vaihtoehdoista koe+laskarit+ht ja pelkk‰ koe "
...
//     protected boolean[] frozenGrp = new boolean[ MAX_GROUP ];
//     {// TESTI!!!
//  	frozenGrp[0] = true;
//     }
    ...
    protected String scale = "K"; // K = 1-5, E = hyv‰ksytty/hyl‰tty
    ...
    /**
     **  T‰ll‰ hetkell‰ k‰sittelyss‰ oleva osasuoritus.
     */
	...
	if ( courseInfo == null ) throw new NullIdException();
...
// 	    System.out.println(rv);
		...
		if ( rv.indexOf("ORA-01400") >= 0 && rv.indexOf("RYHMA_NRO") >= 0 ) {
		    this.msg = "Ryhm‰‰ "+group+" ei ole olemassa.";
		} else {
		    this.msg = rv;
		}
		return false;
	...
	final String[][] partSQLnames =
	{ {"laskarikerta_lkm", "pakolliset_laskarikerta_lkm", "max_laskaripisteet"},
	  {"harjoitustyo_lkm", "pakolliset_harjoitustyo_lkm", "harjoitustyopisteet"},
	  {"valikokeet_lkm",   "pakolliset_koe_lkm",           null} };

	final String[][] minmaxSQLnames =
	{ {"laskaritehtava_lkm", "hyvaksytty_laskarilasnaolo"},
	...
	try {
		...
		if ( part.partDefMod() && ptype < Part.ARVOSANA ) {
		    ...
		    // Alusta tyhj‰t min/max-kent‰t
		    if (p < isNull.length && isNull[p] && part.getNbrOfOfferings() > 0) {
			stmt = con.createStatement();
// 			System.out.println("Alustetaan tyhj‰t min/max-kent‰t...");
		...
		// T‰m‰n voisi hoitaa tehokkaamminkin --> ker‰‰m‰ll‰ listaa 
		// muutoksista suoraan silloin kun muutokset tehd‰‰n
...
// 		System.out.println(sql);
		stmt = con.createStatement();
		if ( stmt.executeUpdate( sql ) != 1 ) rv = false;
		else rv = true;
	    }
	} finally {
	    if ( stmt != null ) {
	...
	final String[] scoreBoundarySQLnames =
	{ "lisapisterajat", "harjoitustyon_pisterajat", null, "arvosanarajat",null,null };
	...
	try {
		...
		while ( su.hasNext() ) {
			...
			this.msg = "Opiskelijan "+s.getLabel()+" pisteiden tallennus ei onnistunut.\n"
			    +"Joko h‰nen ilmoittautumisensa on peruuntunut tai kurssi on j‰‰dytetty.";
			rv = false;
		    }
		    else this.modifyCount++;
		}
	    }
	} finally {
	    if ( pstmt != null ) {
	...
	try {
		...
		switch (result) {
		case -1:
		    this.msg = "Ryhm‰‰ "+newGroup+" ei ole m‰‰ritelty.";
		    break;
		case -2:
		    this.msg = "Opiskelijan voimassaolevaa ilmoittautumista ei lˆytynyt.";
		    break;
		case -3:
		    this.msg = "Ryhm‰n opiskelijalaskurin v‰hent‰minen ep‰onnistui.";
		    break;
		case -4:
		    this.msg = "Ryhm‰n opiskelijalaskurin kasvattaminen ep‰onnistui.";
		    break;
		case -5:
		    this.msg = "Ryhm‰n vaihdon vahvistaminen ep‰onnistui.";
		    break;
		}
		rv = false;
	    }
...
// 	}
	if (this.examDate == null) {
	    this.msg = "J‰‰dytyst‰ ei ole mahdollista suorittaa. "
		+"Anna suoritusp‰iv‰.";
	    return false;
	}
        if (this.getDonName().equals("TUNTEMATON")) {
            this.msg = "J‰‰dytyst‰ ei voi suorittaa koska vastuuhenkilˆ‰ "+
                       "ei ole kunnolla m‰‰ritelty. Ota yhteytt‰ opetushallintoon!";
            return false;
        }

	// Onko suoritusp‰iv‰m‰‰r‰ < j‰‰dytysp‰iv‰m‰‰r‰‰ (sysdate)
	else if (this.examDate.after(Calendar.getInstance())) {
	    this.msg = "J‰‰dytyst‰ ei voi suorittaan ennen kurssin p‰‰ttymist‰. "
		+"Tarkista kurssin suoritusp‰iv‰m‰‰r‰.";
	    return false;
	}
    ...
    public int getCNO() { return this.courseInfo.cno; }
    public MultiValueCounter getCounter() { return new MultiValueCounter(); }

    public float getCredits() { return this.credits; }
    public float getCreditsMax() { return this.creditsMax; }
    public float getNewCreditsMax() { return this.newCreditsMax; }
    public float getNewCredits() { return this.newCredits; }
	...
	if ( this.donLName == null && this.donFName == null ) {
	    ...
	    if ( this.donLName == null ) this.donLName = "";
	    if ( this.donFName == null ) this.donFName = "";
	...
	throws SQLException, ClassNotFoundException {
	if ( this.donFName == null ) getDonName();
	return this.donFName;
    }

    public String getDonLName() 
	throws SQLException, ClassNotFoundException {
	if ( this.donLName == null ) getDonName();
	return this.donLName;
    }

    public int getYear() { return this.courseInfo.year; }
    public String getCCode() { return this.courseInfo.ccode; }
    public String getTerm() { return this.courseInfo.term; }
    public String getType() { return this.courseInfo.type; }
    ...
    public synchronized CourseInfo getCourseInfo() {return this.courseInfo;}

    public synchronized String getLabel() { return this.courseInfo.getLabel(); }
	...
	return grade.getFirstXtrScore();
      ...
      catch (NullIdException nid) {return 1;}
    }
	...
	return grade.getScoreBoundaries();
      ...
      catch (NullIdException nid) {return null;}
    }
	...
	if ( type < 0 || type >= Part.NO_OF_TYPES ) return null;
	...
	for ( ; i < this.parts.size(); i++ ) {
	    ...
	    if ( type == pid ) return part;
	    else if ( type < pid ) break;
	}
    ...
    public synchronized String getScale() { return this.scale; }

    public synchronized String getSelectDescription() {
	String desc = null;
	String[][] names = { { "ryhm‰", "ryhm‰t" }, { "sukunimi", "sukunimet"}, {"opiskelijanumero", "opiskelijanumerot"} };

	for ( int type=0; type < names.length; type++ ) {
	    LinkedList list = null;
	    switch ( type ) {
	    case 0: list = this.group; 
		break;
	    case 1: list = this.lname; 
		break;
	    case 2: list = this.ssn; 
		break;
	    }
		...
		if ( n == 1 ) {
		    if ( desc == null ) desc = names[type][0]+" " + tmp;
		    else desc += ", "+names[type][0]+" " + tmp;
		}
		else if ( n > 1 ) {
		    if ( desc == null ) desc = names[type][1]+" " + tmp;
		    else desc += ", "+names[type][1]+" " + tmp;
		}
	    }
	}

	if ( lnameFrom != null || lnameTo != null ) {
	    if ( desc == null ) desc = "opiskelijat ";
	    else desc += ", opiskelijat ";
	...
	if ( from == null || undefCol == null ) return null;
	...
	// pilkku pois
	if ( rv.length() > 0 ) rv = rv.substring( DELIM.length(), rv.length() );
	...
	/*
	 *  Opiskelijoiden haku ryhm‰n perusteella.
	 */
	...
	while ( itr.hasNext() ) {
       ...
       /*
        * Opiskelijoiden haku henkilˆtunnuksen  perusteella
        */
	...
	/*
	 *  Opiskelijoiden haku sukunimen perusteella annetulta v‰lilt‰.
	 */
	if ( this.lnameFrom != null || this.lnameTo != null ) {
	    cmpFilter += this.lnameFrom+this.lnameTo;

	    // t‰tyykˆ t‰m‰ osa laittaa sulkujen sis‰‰n?
	...
	// J‰rjest‰minen (jos j‰rjest‰misperuste muuttuu, suoritetaan uusi haku)
...
//     	    System.out.println(sql);
		...
		while ( rs.next() ) {
		    ...
		    String[] scoreFields =
			{"laskarisuoritukset", "harjoitustyopisteet", "koepisteet", "arvosana", "laajuus_op", "kielikoodi"};
    ...
    public synchronized String getValue() { return this.courseInfo.getValue(); }
...
// 	    System.out.println("groups loaded");
		...
		if ( freeze != null && freeze.equals("J") ) this.courseInfo.freeze();
		...
		if ( this.scale == null || this.scale.equals("") ) this.scale = "K";

		this.convention = rs.getInt("laskentakaava");
		if ( rs.wasNull() ) this.convention = DEFAULT_CONVENTION; 
...
// 	}
    ...
    public static int negation( int val ) { return 0-val; }
	...
	if ( isFrozen() && student.isFrozen()) {
	    this.msg = "Kertaalleen j‰‰dytetty‰ opiskelijaa ei voi poistaa kurssilta.";
	    return false;
	}
	...
	try {
		...
		case 1:
		    ...
		    break;
		case 2:
		    this.msg = "Ryhm‰n opiskelijalaskurin p‰ivitys ep‰onnistui.";
		    break;
		case 3:
		    this.msg = "Poistamisen vahvistaminen ep‰onnistui.";
		    break;
		}
	    }
	} finally {
	...
	try {
		...
		case 2:
		    ...
		    break;
		case 3:
		    this.msg = "Palauttamisen vahvistaminen ep‰onnistui.";
		    break;
		}
	    }
	} finally {
	...
	} catch ( NullIdException nie ) { return false; }
    }
	...
	if ( c == null ){
	    this.msg = "Suoritup‰iv‰m‰‰r‰ annettu virheellisesti: '"
		+edate+"'. (pp.kk.vvvv)";
	    return false;
	}
	else {
	    ...
	    // Suoritusp‰iv‰m‰‰r‰ < max(sysdate, p‰‰ttymis_pvm) + 2kk (Satu Eloranta)
	    ...
	    // Suoritusp‰iv‰m‰‰r‰ > sysdate - 6kk (Satu Eloranta)
	    lowLimit.add(Calendar.MONTH, -6);

	    if (c.before(lowLimit) || c.after(hiLimit)) {
		this.msg = "Suoritusp‰iv‰m‰‰r‰n tulee olla v‰lill‰ "
		    ...
		    +hiLimit.get(Calendar.YEAR)+" ja ennen kurssin j‰‰dytt‰mist‰.";
		return false;
	    }
	...
	if ( isFrozen() ) {
	   ...
	   return grade.setFirstXtrScore( ms );
        ...
        catch (NullIdException nid) {return false;}
    }
